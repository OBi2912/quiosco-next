...
const increaseQuantify = useStore((state) => state.increaseQuantify )
const decreaseQuantify = useStore((state) => state.decreaseQuantify )
...
const disableDecreaseButton = useMemo(() =>item.quantity === MIN_ITEMS, [item])
  ...
  <div className="space-y-4">
    ...
    <div className="flex gap-5 px-10 py-2 bg-gray-100 w-fit rounded-lg">
        <button
          type="button"
          onClick={() => decreaseQuantify(item.id)}
        ...
        <button
           type="button"
           onClick={() => increaseQuantify(item.id)}
