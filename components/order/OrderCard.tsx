...
export default function OrderCard({ order }: OrderCardProps) {
    ...
    return (
        ...
        >
            ...
            {/* 
                Fixing 'action' prop type: Should only return void or Promise<void>, 
                not Promise<{success/error}>. We'll wrap completeOrder 
                to swallow the result, or move handler to server func for server actions (Next.js 13+). 
            */}
            {!order.status && (
                <form action={async (formData: FormData) => { 
                    await completeOrder(formData); // ignore returned value 
                }}>
                    <input 
                        type="hidden"
